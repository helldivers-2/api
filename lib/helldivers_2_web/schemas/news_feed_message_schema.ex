defmodule Helldivers2Web.Schemas.NewsFeedMessageSchema do
  ...
  def response(),
    do:
      {"Newsfeed message response", "application/json", __MODULE__,
       Helldivers2Web.ApiSpec.default_options()}

  def responses(),
    do:
      {"Newsfeed response", "application/json", %Schema{type: :array, items: __MODULE__},
       Helldivers2Web.ApiSpec.default_options()}

  OpenApiSpex.schema(%{
    description: "Represents a message in the Helldivers 2 newsfeed",
    type: :object,
    properties: %{
      id: %Schema{
        type: :integer,
